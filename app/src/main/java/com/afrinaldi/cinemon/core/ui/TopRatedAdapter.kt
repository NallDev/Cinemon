...
class TopRatedAdapter(private val data: List<ResultsItemTopRated>) : RecyclerView.Adapter<TopRatedAdapter.ViewHolder>() {
    ...
    inner class ViewHolder(private val binding: ItemMoviesBinding) : RecyclerView.ViewHolder(binding.root){
        fun bind(item : ResultsItemTopRated, context : Context){
            with(binding) {
