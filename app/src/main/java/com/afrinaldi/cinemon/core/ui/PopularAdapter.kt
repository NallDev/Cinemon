...
class PopularAdapter(private val data: List<ResultsItemPopular>) : RecyclerView.Adapter<PopularAdapter.ViewHolder>() {
    ...
    inner class ViewHolder(private val binding: ItemMoviesBinding) : RecyclerView.ViewHolder(binding.root){
        fun bind(item : ResultsItemPopular, context : Context){
            with(binding) {
